...
TH1D * RootImprove::CrearTH1Simu(const char* argv, const char* name, int nbin)
{
  double xlow, xup;
  xlow = GetMinimumColumna(argv, "x", "x");
  xup = GetMaximumColumna(argv, "x", "x");
...
TH1D * RootImprove::CrearTH1Simu(const char* argv, const char* name , int nbin, double xlow, double xup)
{
  TH1D *h= new TH1D (name , "", nbin , xlow , xup);
  ifstream file;
  double column1;  
...
{
  h1->GetXaxis()->SetLimits(xlow , xup);
  h1->SetLineColor(1);
  h2->SetLineColor(2);
